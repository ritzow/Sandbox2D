...
float lighting(ivec2 position) {
    return texelFetch(tileLighting, position, 0).r/255.0f;
}
...
void main() {
    if(SMOOTH_LIGHTING) {
        ...
        color = vec4(0, 0, 0, 1.0 - mix(lightBottom, lightTop, ratioY));
    } else {
        color = vec4(0, 0, 0, 1.0 - lighting(ivec2(int(worldCoord.x + 0.5), int(worldCoord.y + 0.5))));
    }
}