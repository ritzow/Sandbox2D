...
public final class ItemEntity extends ClientEntity {
	protected final ClientItem item;
	protected float rotation;
	protected float rotationSpeed;
	
	public ItemEntity(int entityID, ClientItem item) {
		super(entityID);
		this.item = item;
		this.rotationSpeed = (float) (Math.random() < 0.5f ? -(Math.random() * 0.02f + 0.02f) : (Math.random() * 0.02f + 0.02f));
	}
	
	public ItemEntity(int entityID, ClientItem item, float x, float y) {
		super(entityID);
		this.item = item;
		this.positionX = x;
		this.positionY = y;
